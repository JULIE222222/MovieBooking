...
function onMonthChange() {
    currentMonth = parseInt(monthSelect.value); // 드롭다운에서 선택된 월을 숫자로 변환
    updateCalendar(); // 달력을 갱신합니다.
}

// 달력 갱신 함수
function updateCalendar() {
    monthYearDisplay.textContent = `${currentYear}년`; // 현재 년월을 표시

    reserveDate.innerHTML = ""; // 기존 날짜 초기화

    const currentDate = new Date(currentYear, currentMonth); // 현재 년월의 첫째 날
    const firstDayIndex = currentDate.getDay(); // 월의 첫째 날의 요일 인덱스
    const lastDate = new Date(currentYear, currentMonth + 1, 0).getDate(); // 현재 월의 마지막 날짜
    ...
    for (let date = 1; date <= lastDate; date++) {
        ...
        if (buttonDate >= today) { // 오늘 날짜와 오늘 이후의 날짜만 표시
            dateButton.textContent = `${date}일 (${weekOfDay[(firstDayIndex + date - 1) % 7]})`; // 날짜와 요일 표시
            dateButton.onclick = () => selectDate(buttonDate); // 날짜 버튼 클릭 이벤트 처리
            reserveDate.appendChild(dateButton); // 날짜 버튼을 달력에 추가
        }
    }
}
...
function updateSelectedDate(date) {
    document.getElementById("selected-date").textContent = `날짜: ${formatDate(date)}`; // 날짜를 텍스트로 표시
}
